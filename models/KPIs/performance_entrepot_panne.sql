...
WITH volumes_par_entrepot_type_etat AS (
  ...
  GROUP BY m.id_entrepot, m.id_machine, m.type_machine, m.etat_machine
),
...
pannes_par_entrepot_type_etat AS (
  SELECT
    ...
    m.type_machine,
    m.etat_machine,
    SUM(m.temps_arret) AS temps_panne_total
  FROM {{ ref('mrt_fct_machines') }} m
  GROUP BY m.id_entrepot, m.id_machine, m.type_machine, m.etat_machine
)
...
LEFT JOIN pannes_par_entrepot_type_etat p 
  ON e.id_entrepot = p.id_entrepot AND v.id_machine = p.id_machine
