...
-- Objectif : identifier les mois, semaines et jours avec des volumes de vente exceptionnellement élevés
-----------------------------------------------------------------------------------

WITH ventes_par_jour AS (
  -- Étape 1 : Calcul du chiffre d'affaires journalier
  SELECT
    CAST(date_commande AS DATE) AS jour,                               -- Conversion de STRING en DATE
    EXTRACT(YEAR FROM CAST(date_commande AS DATE)) AS annee,
    EXTRACT(MONTH FROM CAST(date_commande AS DATE)) AS mois,
    EXTRACT(WEEK FROM CAST(date_commande AS DATE)) AS semaine,
    EXTRACT(DAYOFWEEK FROM CAST(date_commande AS DATE)) AS jour_semaine,  -- 1 = dimanche

    SUM(quantite * prix) AS ca_journalier
  FROM {{ ref('mrt_fct_commandes') }} f
  JOIN {{ ref('mrt_dim_produits') }} p ON f.id_produit = p.id_produit
  GROUP BY 
    CAST(date_commande AS DATE),
    EXTRACT(YEAR FROM CAST(date_commande AS DATE)),
    EXTRACT(MONTH FROM CAST(date_commande AS DATE)),
    EXTRACT(WEEK FROM CAST(date_commande AS DATE)),
    EXTRACT(DAYOFWEEK FROM CAST(date_commande AS DATE))
),

stats_mensuelles AS (
  -- Étape 2 : Moyenne et écart-type du CA par mois (tous mois confondus)
  SELECT
    mois,
...
stats_hebdomadaires AS (
  -- Étape 3 : Moyenne et écart-type du CA par jour de semaine (tous jours confondus)
  SELECT
    jour_semaine,
...
classement_journalier AS (
  -- Étape 4 : Calcul des z-scores et classements
  SELECT
    v.jour,
    v.annee,
    v.mois,
...
-- Étape finale : sélection des jours "forts"
SELECT
  jour,
  annee,
  mois,
